...
6. **성취기준을 지어내지 않는다.** `standards.yaml` 에 없는 코드는 쓰지 마라. 그 파일에
   `crosscheck: 불일치`·`미대조` 로 적힌 코드도 **쓰지 마라** — 사람이 NCIC 원문으로
   확정하기 전까지는 태그로 쓸 수 없다. 그런 코드가 필요한 자리는 `uncertain` 으로 올린다.
   **`교사 확정` 은 다르다** — 지도서 곁주 코드 오식을 사람이 이미 바로잡은 것이라
   `일치` 와 같은 신뢰도이고, 이것을 근거로 범위 이탈 판정을 내리면 안 된다.
...
기준은 하나다: `inverted_text` 를 **문면 그대로** 읽었을 때 참이 될 경우가 하나라도
있는가. 있으면 `false`, 없으면 `true`. 판정할 때 다음 셋을 꼭 확인해라.

- **뒤집은 문장과 역(逆) 명제를 헷갈리지 마라.** 「A → B」의 뒤집은 문장은 「A → not B」이고
  「B → A」가 아니다. 「간섭무늬가 없다고 해서 슬릿을 하나만 연 것은 아니므로 false」 같은
  `invertible_reason` 은 역을 깬 것이다 — 이 잘못이 한 단원에서 79건까지 나온 적이 있다.
- **`form.type` 은 근거가 되지 못한다.** 「분류형이라 방향이 없다」·「까닭을 대는 명제라
  방향이 없다」는 사유가 아니다. 분류형이야말로 두 이름을 맞바꾸면 반드시 거짓인 일이 잦다.
- **조건이 `inverted_text` 문면에 있는가.** D1 은 `scope`·`condition` 을 끌고 가지 않는다.
  조건이 문면에서 떨어져 나가면 참이 될 길이 열려 판정이 무너진다. 이때는 `false` 로
  내리라고 하기 전에 **조건을 문면에 넣어 살리는 쪽**을 제안해라.

`invertible` 비율이 다른 단원과 크게 다르면(대개 45~55 %) 한쪽으로 기운 것이니
전수로 다시 재고, 바꿔야 할 건을 낱낱이 목록으로 내라.
    ...
    "findings": [
      {
        ...
        "verdict": "defect",
        "checks": [2],
        "reason": "inverted_text 「1 mol 에 든 입자 수는 물질마다 다르다」가 참이 될 경우가 없는데 invertible: false 로 잠겨 있다. invertible_reason 이 역 명제(「입자 수가 같으면 1 mol 이다」)를 반박하고 있다",
        "suggest": "invertible 을 true 로 올리고 invertible_reason 을 뒤집은 문장 기준으로 다시 쓴다"
      }
    ],
