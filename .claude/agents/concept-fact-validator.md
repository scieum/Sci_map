...
| 2 | `invertible` 오판정 | **뒤집은 문장이 반드시 거짓이 되는가.** 이것이 유일한 기준이다 |
...
②에서 `form.type` 은 **어디를 먼저 볼지 알려 주는 표지일 뿐 판정 근거가 아니다.**
`classification`·`conditional` 에 `true` 가 붙어 있으면 대개 오판정이지만, 두 이름을
맞바꾸는 분류형처럼 뒤집으면 반드시 거짓이 되는 것도 있다 — 그런 것은 `ok` 다.
반대로 `direct` 여도 뒤집은 문장이 참일 수 있으면 `defect` 다. 정의를 되풀이한 문장,
계산 절차, 비의 상등이 그 자리다. **type 을 보고 판정하지 말고 뒤집어 읽고 판정하라.**
