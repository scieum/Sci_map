...
- `curriculum` — 이 명제가 걸리는 성취기준. `crosscheck` 가 `일치` 또는 `교사 확정` 인 코드만.
  `교사 확정` 은 지도서 곁주 코드 오식을 사람이 바로잡은 것이라 `일치` 와 같은 신뢰도다

**`invertible` 판정 요령**

기준은 **하나**다: `inverted_text` 를 **문면 그대로** 읽었을 때 그 문장이 참일 수 있는
경우가 **하나라도** 있는가. 있으면 `false`, 없으면 `true`.

- "증기 압력이 크면 분자 간 힘이 작다" → 뒤집으면 "크면 크다", 반드시 거짓 → `true`
- "분자량이 크면 대체로 끓는점이 높다" → 예외가 있어 뒤집어도 항상 거짓은 아니다 → `false`

**★가장 흔한 잘못 — 뒤집은 문장 대신 역(逆) 명제를 시험한다.** 네 단원에서 되풀이됐다.
「A → B」의 **뒤집은 문장**은 「A → not B」다. 「B → A」(역)가 아니다.

> 「슬릿 두 개를 다 열면 간섭무늬가 생긴다」
> 뒤집은 문장 = 「슬릿을 한쪽만 열어도 간섭무늬가 생긴다」 → 교과서와 어긋나 반드시 거짓 → **`true`**
> 역 = 「간섭무늬가 있으면 슬릿을 두 개 연 것이다」 → 이건 판정 대상이 아니다

「간섭무늬가 없다고 해서 슬릿을 하나만 연 것은 아니므로 false」 같은 사유를 쓰고 있다면
역을 깬 것이다. `invertible_reason` 에는 **뒤집은 문장을 그대로 적고** 그것이 왜 거짓인지를 써라.

**★`form.type` 은 판정 근거가 아니다.** 「까닭을 대는 명제라 방향이 없다」(causal),
「분류라 방향이 없다」(classification) 는 사유가 되지 못한다. 분류형이야말로 두 이름을
맞바꾼 문장이 반드시 거짓인 일이 잦다 — 「레이저는 결맞고 LED 는 결맞지 않다」를 맞바꾸면
반드시 거짓이다.

**조건은 `inverted_text` 안에 들어가야 한다.** D1 은 `scope`·`condition` 을 끌고 가지 않고
`inverted_text` 만 떼어 OX 지문으로 쓴다. 조건이 문면에서 떨어지면 참이 될 길이 열려
판정이 무너진다 — 「전기장 방향으로 잰 거리가 멀수록 전위차가 크다」에서 "전기장 방향으로 잰"이
빠지면 등전위면 위의 두 점이 곧 반례가 된다. **조건을 문면에 넣어 `true` 를 살리는 쪽을
먼저 시도하고**, 그래도 안 되면 `false` 로 내려라.

**같은 개념을 다른 단원이 이미 다뤘다면 그쪽 판정을 보고 맞춰라.** 한 명제가 단원마다
다르게 잠겨 있으면 D1 이 서로 어긋나는 OX 를 낸다.

기계적인 "애매하면 `false`" 는 쓰지 마라 — 그 습관으로 한 단원의 `invertible` 이 16 % 까지
떨어져 D1 이 만들 문항이 말라붙은 일이 있다. 위 절차를 밟고도 참이 될 경우를 실제로
하나 댈 수 있을 때에만 `false` 다.
