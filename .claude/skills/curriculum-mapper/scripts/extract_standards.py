...
CODE_RE = re.compile(
    r"\[\s*12\s*(?:물\s*에|화\s*학|반\s*응|세\s*포|생\s*과|유\s*전|물\s*리|지\s*구|역\s*학|전\s*자|지\s*시|행\s*우)\s*\d{2}\s*-\s*\d{2}\s*\]"
...
COURSES = {
    ...
    # 물리학(432쪽)은 표준 배치다 — 총론 '나. 성취기준' 절(21~23쪽)에 띄어쓰기가
    #   살아 있어 그쪽이 1차이고, 각론의 성취기준별 평가 자료(242~338쪽)가 대조본이다.
    "12물리": {"name": "물리학", "type": "일반선택", "primary": (21, 23), "cross": (242, 338),
              "areas": {"01": "힘과 에너지", "02": "전기와 자기", "03": "빛과 물질"}},
    # 지구과학(496쪽)도 표준 배치다 — 총론 '나. 성취 기준' 절(26~28쪽)에 띄어쓰기가 살아 있고,
    #   각론의 대단원 개관 쪽(Ⅰ 64 · Ⅱ 156 · Ⅲ 228)이 대조본이다.
    "12지구": {"name": "지구과학", "type": "일반선택", "primary": (26, 28), "cross": (64, 233),
              "areas": {"01": "대기와 해양의 상호작용", "02": "지구의 역사와 한반도의 암석",
                        "03": "태양계 천체와 별과 우주의 진화"}},
    # 역학과 에너지(320쪽)도 표준 배치다 — 총론 '나. 성취기준' 절(19~21쪽)에 띄어쓰기가 살아 있고,
    #   각론의 '최소 성취 수준 평가' 쪽(160~242, 소단원마다 한 쪽씩)이 대조본이다.
    #   각론은 코드를 '[12역학01- 01]' 처럼 숫자 사이를 띄어 찍으므로 CODE_RE 의 \s* 가 필요하다.
    "12역학": {"name": "역학과 에너지", "type": "진로선택", "primary": (19, 21), "cross": (160, 242),
              "areas": {"01": "시공간과 운동", "02": "열과 에너지", "03": "탄성파와 소리"}},
    # 전자기와 양자(336쪽)도 같은 저자·같은 배치다 — 총론 '나. 성취기준' 절(19~21쪽)이 1차,
    #   각론의 '최소 성취 수준 평가' 쪽(170~260, 소단원마다 한 쪽)이 대조본이다.
    "12전자": {"name": "전자기와 양자", "type": "진로선택", "primary": (19, 21), "cross": (170, 260),
              "areas": {"01": "전자기적 상호작용", "02": "빛과 정보 통신", "03": "양자와 미시 세계"}},
    # 지구시스템과학(320쪽)은 「지구과학」과 같은 저자·판형이라 표준 배치다 — 총론 '나. 성취 기준'
    #   절(24~26쪽)이 1차, 각론의 대단원 개관 쪽(60~161)이 대조본이다.
    "12지시": {"name": "지구시스템과학", "type": "진로선택", "primary": (24, 26), "cross": (60, 161),
              "areas": {"01": "지구 탄생과 생동하는 지구", "02": "해수의 운동",
                        "03": "강수 과정과 대기의 운동"}},
    # 행성우주과학(336쪽)도 「지구과학」·「지구시스템과학」과 같은 저자·판형이라 표준 배치다 —
    #   총론 '나. 성취 기준' 절(24~26쪽)이 1차, 각론의 대단원 개관 쪽(60~169)이 대조본이다.
    "12행우": {"name": "행성우주과학", "type": "진로선택", "primary": (24, 26), "cross": (60, 169),
              "areas": {"01": "우주 탐사와 행성계", "02": "태양과 별의 관측",
                        "03": "은하와 우주"}},
}
...
def norm_for_compare(s: str) -> str:
    ...
    #
    # ★공백을 **먼저** 지우고 나서 라벨을 걷어낸다. 같은 라벨을 판형마다 다르게
    #   띄어 쓰기 때문이다 — 물리학 지도서 각론은 '교육과정 성취 기준'(성취와 기준
    #   사이에 공백)으로 찍어, 공백이 살아 있는 채로 맞추면 ROW_LABELS 의
    #   '교육과정 성취기준' 과 어긋나 라벨이 문장 안에 남는다. 그러면 18건이 전부
    #   '불일치' 로 올라와 정작 사람이 봐야 할 진짜 이견이 묻힌다.
    s = re.sub(r"[\s·ㆍ‧・･·,]", "", s)
    # 긴 라벨부터 지운다 — '성취기준' 을 먼저 지우면 '교육과정 성취기준' 이 더는 맞지 않아
    # '교육과정' 이 문장에 남는다.
    # 쉼표와 가운뎃점도 공백과 같은 부류로 본다 — 총론 '물질의 물리적, 화학적 성질' 과
