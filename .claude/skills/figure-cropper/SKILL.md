...
### 예외 — 단원 전체에 캡션이 하나도 없을 때 (2026-09-08 교사 결정)

위 규칙은 **캡션 있는 그림과 없는 그림이 섞여 있을 때** 무엇을 고를지 정하는 규칙이다.
단원 전체에 번호·캡션이 **0건**이면 이야기가 다르다 — 규칙을 지키면 그 단원 카드가
전부 그림을 못 받는데, 그것은 규칙이 막으려던 문제(출처 불명)가 아니라 다른 문제다.

통합과학1 Ⅰ단원(`isci1-1`)이 그 경우다. 26쪽에 캡션이 0건인데 규모 인포그래픽·
아날로그/디지털 신호 도해처럼 카드 명제를 곧바로 받쳐 주는 삽화는 풍부하다.
교사가 이 단원에 한해 예외를 허용했다.

예외를 쓸 때 **추적성을 대신 확보한다** — 캡션이 하던 일을 다른 것으로 메운다:

- `figure_no` 는 `null` 로 두지 말고 `"번호 없음"` 으로 적는다 (조사 안 함과 구분)
- `rights.source` 에 **쪽과 지면 위치**를 적는다 — `"027쪽 본문 하단 도해"`.
  학생도 교사도 그 그림을 교과서에서 되찾을 수 있어야 한다
- 워터마크 띠(발행사·수업 목적 이용 문구)는 그대로 붙는다

**단원 하나에 한 번 내리는 결정이다.** 캡션이 있는 단원에서 무캡션 삽화를 끌어오는 데
쓰지 마라 — 그건 위 규칙이 그대로 막는다.
