...
| | `mate-1` 물질의 세 가지 상태 | `mate-2` 용액의 성질 | `mate-3` 화학 변화의 자발성 | `mate-4` 반응 속도 |
|---|---|---|---|---|
| 개념 카드 | **30장** | **24장** | **27장** | **24장** |
| 관계 명제 | 89 (`invertible` 58) | 72 (`invertible` 53) | 82 (`invertible` 53) | 72 (`invertible` 52) |
| 오개념 | 60 (전부 `source: llm`) | 49 (전부 `source: llm`) | 55 (전부 `source: llm`) | 52 (전부 `source: llm`) |
| 교과서 자산 | 45장 → 카드 29/30 | 26장 → 카드 24/24 | 35장 → 카드 26/27 | 18장 → 카드 23/24 |
| 단계 | **C8 통과** · C9 대기 | **C8 통과** · C9 대기 | **C8 통과** · C9 대기 | **C8 통과** · C9 대기 |

그림 없는 카드 셋은 의도다 — 동소체·효소는 교과서에 삽화가 없고(§0.3), 끓는점은 교사 판단이다.
...
네 단원 모두 **C8 을 통과해 C9(배포) 앞에 서 있다.** C9 는 Supabase 가 붙어야 열린다.
C5(범위 태깅)·C6(명제 결함 탐지)는 담당 서브에이전트(`concept-fact-validator`)가
아직 없어 네 단원 모두 건너뛰었다 — CLAUDE.md §3 과 실제가 어긋나는 지점이다.
...
due(다음 볼 날)가 있고, 매 응답이 그 개념의 due 를 민다 — 잘 알수록 멀리, 틀리면 내일.
응답 시간도 등급에 들어간다(느리게 맞히면 Hard).

오늘의 개념 집합은 **하루 한 번** 뽑는다: due 가 지난 개념 전부(복습, 이른 순) +
아직 안 본 개념을 백로그 순서로 **10개까지**(신규 상한, 교사 결정). 세 유형 세션(OX ·
단답 · 선택형)이 같은 집합을 나눠 쓴다 — 같은 개념을 다른 방향으로 다시 묻는다.

CLAUDE.md 는 py-fsrs 를 지정했다. 앱 안에서 돌리려면 같은 알고리즘의 TS 구현이 필요해
`ts-fsrs`(MIT, FSRS-5)를 쓴다. Supabase 가 붙어 서버 스케줄링으로 가면 py-fsrs 로
옮기되, 위 규칙은 그대로 둔다. 저장은 지금 localStorage 다 — `store.ts` 만 갈아끼운다.
