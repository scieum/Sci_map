...
|---|---|---|
| 개념 카드 | **30장** (소주제 6개) | **24장** (소주제 5개) |
| 관계 명제 | 89개 (`invertible` 58) | 72개 (`invertible` 53) |
| 오개념 | 60개 | 49개 (전부 `source: llm`) |
| 교과서 그림 | 25장 크롭 → 카드 30/30 | 22장 크롭 → 카드 24/24 |
| 파이프라인 단계 | **C7 완료 → C8 대기** | **C7 완료 → C8 대기** |

두 단원 모두 **C8 사람 게이트에서 멈춰 있다.** C5(범위 태깅)·C6(명제 결함 탐지)는
담당 서브에이전트(`concept-fact-validator`)가 아직 없어 건너뛰었다 — CLAUDE.md §3 과
실제가 어긋나는 지점이다.
